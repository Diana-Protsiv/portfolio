...
В исходном логе насчитывается 243 тыс. 713 событий и 7 тыс. 551 уникальных пользователей. В среднем на одного пользователя приходится 32 события. Не более 1% пользователей совершили более 200 событий. Вероятно, большое количество событий (более 1 тыс.) было совершено не реальными пользователями, а ботами. Пользователи, совершившие более 200 действий, были определены как аномальные. Был создан новый датафрейм, из которого был исключён 1% пользователей с наибольшим количеством событий. Также было установлено, что в исходном логе содержатся данные за две недели. Самая ранняя дата — 25 июля 2019 года, самая поздняя — 7 августа того же года. Поскольку данные за июль неполные, они были отброшены. В результате остались данные только за первые 7 дней августа 2019 года. После удаления аномальных пользователей и неактуальных данных в файле осталось 206 тыс. 615 событий и 7 тыс. 534 уникальных пользователя. Доля удалённых пользователей составила всего 1.23%. Доля удалённых событий — 15.22%. После удаления данных в среднем на одного пользователя стало приходиться 28 событий, все три экспериментальные группы сохранились.
...
При проверке статистических гипотез было выбрано распространённое значение 0.05 в качестве уровня статистической значимости. Если бы был использован более низкий уровень значимости 0.01, то результаты не изменились бы, так как все полученные p-значения больше 0.05, значит, они были бы больше и 0.01. При уровне значимости 0.05 можно ошибочно отклонить нулевую гипотезу в 5% случаев, если она верна. Если бы был применён уровень значимости 0.1, то уже в 10% случаев можно было бы допустить ошибку первого рода.
